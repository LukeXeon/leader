...
public class NaviViewModel2 extends AndroidViewModel implements OnBackPressedCallback
{
    ...
    public NaviViewModel2(@NonNull Application application)
    {
        super(application);
    ...
    public void query(InputTip inputTip)
    {
        ...
        singleTask.execute(() -> {
            ...
            List<FutureTask<Point>> tasks = Arrays.asList(
                    ...
                    new FutureTask<>(() -> loadTarget(inputTip)));
    ...
    @WorkerThread
    private Point loadTarget(InputTip inputTip)
    {
        PoiSearch.Query query = new PoiSearch.Query(inputTip.text, "");
        query.setDistanceSort(false);
        query.requireSubPois(true);
        PoiSearch poiSearch = new PoiSearch(getApplication(), query);
        try
        {
            PoiItem item = poiSearch.searchPOIId(inputTip.poiId);
    ...
    public RouteInfo getRouteInfo(int id)
    {
